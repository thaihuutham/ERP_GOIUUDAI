...
- Endpoint chuẩn sau khi hợp nhất route:
  ...
  - `DELETE /api/v1/crm/customers/:id` (soft-skip, không xóa cứng)
  - `POST /api/v1/crm/customers/import` (admin only, upsert theo Excel rows)
...
| `status` | `CustomerCareStatus` | Có (default `MOI_CHUA_TU_VAN`) | Trạng thái CSKH chuyên biệt cho vận hành tư vấn/chăm sóc | Tạo/sửa CRM, soft-skip (`SAI_SO_KHONG_TON_TAI_BO_QUA_XOA`), import Excel, merge khách | `/modules/crm`, `/modules/sales`, lọc campaign Zalo |
| `zaloNickType` | `CustomerZaloNickType` | Có (default `CHUA_KIEM_TRA`) | Loại khả năng liên lạc nick Zalo của khách | Chỉnh tay CRM, auto-update sau campaign, import Excel | `/modules/crm`, `/modules/zalo-automation/campaigns` |
...
- Index hiệu năng:
  ...
  - `tenant_Id + email`
  - `tenant_Id + status`
  - `tenant_Id + zaloNickType`
  - `tenant_Id + customFieldSchemaVersion`

## 5. Enum nghiệp vụ mới (Customer)

### 5.1 `CustomerCareStatus`
...
- `DELETE /crm/customers/:id` không xóa bản ghi, chỉ chuyển `status = SAI_SO_KHONG_TON_TAI_BO_QUA_XOA`.
